 ...
 * ================================================= */
if ($is_editable) {
    echo '<form action="news.php" method="post" id="formPost" onsubmit="return isReady(\'n_title\',\''.__('You need to give a title.').'\')">';
    ...
    if ($news->cats->nbRow() >= 1) {
        ...
        while (!$news->cats->EOF()) {
            ...
            if ($news->cats->f('categoryasso_type') != PX_RESOURCE_CATEGORY_MAIN) {
                echo ' - <a href=\'news.php?delcat=1&amp;resource_id='
                    .$news->f('resource_id').'&amp;n_category_id='
        ...
        echo "</ol>\n\n";
        echo '<p><span class="nowrap"><label for="n_category_id" '
            .'style="display:inline">'.__('Add in another category').' '
            .$m->HelpLink('news', 'h-category').'</label><br />'."\n";
        echo form::combobox('n_category_id',$arry_cat, '', 1)."\n";
        ...
        echo '<span class="nowrap"><label for="n_category_id" '
            .'style="display:inline">'.__('Category').' '
            .$m->HelpLink('news', 'h-category').'</label>'."\n";
        echo form::combobox('n_category_id', $arry_cat, 
                            $news->f('category_id'), 
    ...
    if (config::f('comment_support') == 2) {
        ...
        echo form::combobox('n_comment_support', 
                            ...
                            $m->user->getPref('news_comment_support'), 4);
        echo "</span>\n\n";
    } else {
    ...
    echo form::textArea('n_content', 60, 
                        ...
                        $news->getUnformattedContent('description'), 7, 
                        'style="width:100%"');
    ...
    echo '</label>';
    echo form::textArea('n_subject', 60, 4, $news->f('subject'), 8,
                        'style="width:100%"');
    echo "</p>\n";
    ...
    echo form::textField('n_titlewebsite', 50, 255, 
                         $news->details->f('news_titlewebsite'), 9, 
                         'style="width:100%"');
    echo "</p>\n";
    ...
    echo form::textField('n_linkwebsite', 50, 255, 
                         $news->details->f('news_linkwebsite'), 10, 
                         'style="width:100%"'); 
    echo '</p></div>'."\n\n";
    ...
    if ($news->f('resource_id') > 0) {
        ...
        echo ' '.form::datetime('n_dt', $news->getArrayDate('publicationdate'),
                                11)."<br />\n";
        $noenddate_style = ($news->isDateEOT('enddate')) ? 'style="display: none"' : '';
