...
if (empty($_REQUEST['dir'])) {
    ...
    if (!empty($current_dir)) {
        ...
        $env  = ($m->user->getPref('xmedia_current_page')) ? $m->user->getPref('xmedia_current_page') : 1;
    ...
    if (!file_exists($up_dir.'/'.$px_file)) {
        ...
        if (!empty($_GET['del']) && $_GET['del'] == 1) {
            if (@unlink($up_dir.'/'.$px_file) === false) {
                ...
                @unlink($up_dir.'/thumb/'.md5($px_file).'.jpg');
            ...
            $thumb = $up_dir.'/thumb/'.md5($px_file).'.jpg';
...
if ($is_writable && !empty($_POST['create']) && (false === $m->error())
    ...
    if (!empty($_POST['new_folder'])) {
        ...
        if (preg_match('/[^A-Za-z0-9\-\_]/', $new_dir) or ($new_dir == 'thumb')) {
            ...
            if (@mkdir($up_dir.'/'.$current_dir.$new_dir, 0777)) {
                $m->setMessage(__('The folder was successfully created, you can already add files and images in.'));
                header('Location: xmedia.php?dir='.rawurlencode($current_dir.$new_dir).'&mode='.$mode);
                exit();
...
<form enctype="multipart/form-data" action="xmedia.php" method="post" onsubmit="return isReady('up_file','<?php echo __('Error: No file.') ?>')">
<fieldset><legend><?php   echo __('Add a file to the current folder'); ?></legend>
<p><label for="up_file"><?php  echo sprintf( __('Choose a file (maximum size %s)'), prettySize($_PX_config['max_upload_size'])); ?> <?php echo $m->HelpLink('xmedia', 'h-add-file'); ?></label>
...
<input type="hidden" name="dir" value="<?php echo $current_dir; ?>" /></p><p>
<input class="submit" type="submit" value="<?php echo  __('Add the file'); ?>" /></p>
</fieldset>
</form>
<?php endif; ?>
...
if ($is_dir) {
    ...
    if (strlen($current_dir) > 0) {
        ...
        $rep_list[$j]['url'] = config::f('rel_url_files').'/'.$parent_dir;
    ...
    while(false !== ($entry = $D->read()))
    {
        if($entry != '.' && $entry != '..' && $entry != '.htaccess' && $entry != '.htpasswd') {
            if (!is_dir($up_dir.'/'.$current_dir.$entry)) {
                $img_list[$i]['url'] = config::f('rel_url_files').'/'
                    .$current_dir.$entry;
                ...
                $rep_list[$j]['url'] = config::f('rel_url_files').'/'
                    .$current_dir.$entry;
...
$objLum = new lum($env, 'line_file', $img_list, 0, 12);
$m->user->savePref('xmedia_current_page', $objLum->env, $_SESSION['website_id'], true);
...
if ($is_dir) {
    ...
    echo '<p class="small">';
    if (strlen($current_dir) > 0) {
        echo '<a href="xmedia.php?dir='.$parent_dir.'&amp;mode='.$mode
    ...
    if ($is_writable &&  (false === $m->error())
        && (auth::asLevel(PX_AUTH_ADVANCED)) ) {
        ?><form action='xmedia.php' method='post'>
        <input type='hidden' name='dir' value='<?php echo $current_dir; ?>' />
        <input type='hidden' name='mode' value='<?php echo $mode; ?>' />
        <p>
        <span class="nowrap"><label for="new_folder"
        style="display:inline"><?php  echo __('Create a sub-folder'); ?> <?php echo $m->HelpLink('xmedia', 'h-sub-folder'); ?></label>
        <?php echo form::textField('new_folder', 15, 30, '', '', ''); ?>
        <input name="create" type="submit" class="submit" value="<?php  echo __('Create'); ?>" />
        </span></p>
        </form>
        <?php
